...
/**
	...
	* @version     1.0.0
**/
if ( !class_exists('vel_woo_slider_widget') ) {
	class vel_woo_slider_widget extends WP_Widget {
		/**
		 * Widget setup.
		 */
		function __construct(){
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'vel_woo_slider_widget', 'description' => __('SLT Woo Slider', 'vel_maxive') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vel_woo_slider_widget' );

			/* Create the widget. */
			parent::__construct( 'vel_woo_slider_widget', __('SLT Woo Slider widget', 'vel_maxive'), $widget_ops, $control_ops );
		...
		**/
		function WS_integrateWithVC(){
			...
			vc_map( array(
			  "name" => __( "SLT Woo Slider", TEXTDOMAIN ),
			  "base" => "woo_slide",
			  ...
			  "params" => array(
				 ...
				 array(
				  ...
				  array(
					...
					"param_name" => "layout",
					"value" => array( 'Layout Default' => 'layout1', 'Layout item no margin'=>'layout1_nomargin', 'Layout Featured' => 'layout2', 'Layout Top Rated' => 'layout3', 'Layout Best Sales' => 'layout4', 'Layout Child Category' => 'childcat', 'Layout Child Category1' => 'childcat1' ),
		...
		**/
		function WS_Shortcode( $atts, $content = null ){
			...
			ob_start();		
			if( $layout == 'layout1' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/default.php' );
				
			}elseif( $layout == 'layout2' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/featured.php' );			
			}
			elseif( $layout == 'layout3' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/toprated.php' );			
			}
			elseif( $layout == 'layout4' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/bestsales.php' );			
			}
			elseif( $layout == 'childcat' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/childcat.php' );			
			}
			elseif( $layout == 'childcat1' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/childcat1.php' );			
			}
			elseif( $layout == 'layout1_nomargin' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider/default_item_nomargin.php' );			
			}
		...
		**/
		...
		protected function getTemplatePath($tpl='default', $type=''){
			$file = '/'.$tpl.$type.'.php';
			$dir =	plugin_dir_path(dirname(__FILE__)).'/themes/vel-slider';
