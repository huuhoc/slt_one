...
class vel_woo_widget_layered_nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'sellertemplate' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = __( 'SLT WooCommerce Layered Nav', 'sellertemplate' );
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'sellertemplate' ),
				'label' => __( 'Title', 'sellertemplate' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'sellertemplate' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'sellertemplate' ),
				'options' => array(
					'list'     => __( 'List', 'sellertemplate' ),
					'dropdown' => __( 'Dropdown', 'sellertemplate' )
				)
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'sellertemplate' ),
				'options' => array(
					'and' => __( 'AND', 'sellertemplate' ),
					'or'  => __( 'OR', 'sellertemplate' )
				)
			),
		);
	}
		...
		if ( 0 < count( $terms ) ) {
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'sellertemplate' ), wc_attribute_label( $taxonomy ) ) . '</option>';
