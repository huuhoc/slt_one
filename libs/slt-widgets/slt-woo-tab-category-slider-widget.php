<?php
/**
 SLT Woo Tab Category Slider Widget 
 ...
 */
 if ( !class_exists('vel_woo_tab_cat_slider_widget') ) {
	class vel_woo_tab_cat_slider_widget extends WP_Widget {

		/**
		 * Widget setup.
		 */
		function vel_woo_tab_cat_slider_widget() {
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'vel_woo_tab_slider', 'description' => __('SLT Woo Tab Slider', 'yatheme') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vel_woo_tab_slider' );

			/* Create the widget. */
			parent::__construct( 'vel_woo_tab_slider', __('SLT Woo Tab Slider widget', 'yatheme'), $widget_ops, $control_ops );
		...
		**/
		function SC_integrateWithVC(){
			...
			$term = array( __( 'All Category Product', 'js_composer' ) => '0' );
			foreach( $terms as $cat ){
				$term[$cat->name] = $cat->term_id;
			}
			
			vc_map( array(
			  "name" => __( "Woocommerce Tab Category Slider", TEXTDOMAIN ),
		...
		**/
		function SC_WooTab( $atts, $content = null ){
			...
			ob_start();		
			if( $layout == 'default' ){
				include( VEL_WOO_THEMES.'vel-woo-tab-category-slider/default.php' );
			}
			if( $layout == 'layout2' ){
				include( VEL_WOO_THEMES.'vel-woo-tab-category-slider/layout-2.php' );
			}
		...
		protected function getTemplatePath($tpl='default', $type=''){
			$file = '/'.$tpl.$type.'.php';
			$dir = plugin_dir_path(dirname(__FILE__)).'/themes/vel-woo-tab-category-slider';
