<?php
/**
	* SLT Woocommerce Countdown Slider
	* Register Widget Woocommerce Countdown Slider
	* @author 		sellertemplate
	* @version     1.0.0
**/
if ( !class_exists('vel_woo_slider_countdown_widget') ) {
	class vel_woo_slider_countdown_widget extends WP_Widget {
		/**
		 * Widget setup.
		 */
		function __construct(){
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'vel_woo_slider_countdown_widget', 'description' => __('SLT Woo Slider Countdown', 'yatheme') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vel_woo_slider_countdown_widget' );

			/* Create the widget. */
			parent::__construct( 'vel_woo_slider_countdown_widget', __('SLT Woo Slider Countdown Widget', 'yatheme'), $widget_ops, $control_ops );
		...
		public function Get_Product_Countdown(){		
			...
			$p_slug = array( __( 'All Products', TEXTDOMAIN ) => '' );
		...
		**/
		function WS_Countdown_integrateWithVC(){
			...
			vc_map( array(
			  "name" => __( "YA Woocommerce Countdown Slider", TEXTDOMAIN ),
			  ...
			  "params" => array(
				 array(
				  ...
				  array(
					"type" => "dropdown",
					"heading" => __( "Select Product", TEXTDOMAIN ),
					"param_name" => "product_select",
					"value" => $this->Get_Product_Countdown(),
					"description" => __( "Select Product for layout countdown 1", TEXTDOMAIN )
				 ),
				 array(
					"type" => "attach_image",
					...
					"param_name" => "image",
					"value" => '',
					"description" => __( "Select image for layout countdown 1", TEXTDOMAIN )
				 ),	
				 array(
					"type" => "dropdown",
				 ...
				 array(
				  ...
				  array(
					...
					"param_name" => "layout",
					"value" => array( 'Layout Default' => 'layout1', 'Layout Coutdown 1' => 'layout2', 'Layout Coutdown 2' => 'layout3' ),
		...
		**/
		function WS_Shortcode_Countdown( $atts, $content = null ){
			...
			ob_start();		
			if( $layout == 'layout1' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-countdown-slider/countdown.php' );
				
			}elseif( $layout == 'layout2' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-countdown-slider/countdown1.php' );			
			}
			elseif( $layout == 'layout3' ){
				include( plugin_dir_path(dirname(__FILE__)).'/themes/vel-countdown-slider/countdown2.php' );			
			}
