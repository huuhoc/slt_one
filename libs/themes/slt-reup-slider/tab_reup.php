...
<div id="<?php echo 'reup_'.esc_attr( $widget_id ); ?>" class="product-ur">
	<div id="tab_<?php echo esc_attr( $widget_id ); ?>" class="tabbable tabs">
		...
		<?php if( count( $related ) > 0 ){ ?>
			<li class="active">
				<a href="#single_related" data-toggle="tab"><?php _e( 'Related Products', TEXTDOMAIN ); ?></a>
			</li>
		<?php } ?>
		<?php if( count( $upsells ) > 0 ){ ?>
			<li>
				<a href="#single_upsell" data-toggle="tab"><?php _e( 'Upsell Products', TEXTDOMAIN ); ?></a>
			</li>
		<?php } ?>
		</ul>
...
<?php }else{ ?>
	<div class="alert alert-warning">
		...
		<?php esc_html_e( 'This product not have upsell or related!', TEXTDOMAIN ); ?>
	</div>
<?php } ?>